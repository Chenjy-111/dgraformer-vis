...
from dgraudit.cli.validate_pattern import benjamini_hochberg, impact_metrics, sha256
...
def main():
    ...
    cp,rp=Path(a.config).resolve(),Path(a.registry).resolve();cfg=json.loads(cp.read_text());reg=json.loads(rp.read_text());ds=reg["datasets"]["ETTh1"];root=Path(reg["source_root"]);checkpoint=root/"checkpoints"/ds["setting"]/"checkpoint.pth";data=root/ds["root_path"]/ds["data_path"]
    ...
    adapter=DGraFormerAdapter(str(root),"ETTh1",reg["common"],ds,reg["random_seed"]);adapter.load_checkpoint(str(checkpoint));stages=adapter.extract_graph_stages({"current_epoch":cfg["current_epoch"]})["windows"]
    relations=[]
    ...
    for sample in cfg["samples"]:
      ...
      for e in sorted(focal):
        rel=next(x for x in relations if tuple(x["edge"])==e);eligible=[x for x in relations if tuple(x["edge"])!=e];eligible.sort(key=lambda x:(abs(len(x["windows"])-len(rel["windows"])),abs(x["mean_weight"]-rel["mean_weight"]),x["edge"]));pool=eligible[:cfg["control_matching"]["nearest_relations"]];seed=cfg["control_matching"]["random_seed"]+len(records);rng=np.random.default_rng(seed);sampled=[pool[int(rng.integers(0,len(pool)))] for _ in range(cfg["control_matching"]["repetitions"])];imp=np.asarray([metrics[tuple(x["edge"])]["prediction_delta_abs"] for x in sampled]);fm=metrics[e];emp=float((1+np.sum(imp>=fm["prediction_delta_abs"]))/(len(imp)+1));pct=float(100*np.mean(imp<fm["prediction_delta_abs"])+50*np.mean(imp==fm["prediction_delta_abs"]));br=np.random.default_rng(seed+100000);dist=fm["prediction_delta_abs"]-br.choice(imp,size=(cfg["bootstrap_repetitions"],len(imp)),replace=True).mean(1);alpha=1-cfg["confidence_level"]
        ...
        records.append({"id":f"etth1_global_s{sample}_edge_{e[0]}_{e[1]}","sample":sample,"edge":list(e),"retained_windows":rel["windows"],"exposed_windows":exposure,"affected_exposed_windows":sorted(set(rel["windows"])&set(exposure)),"mean_weight":rel["mean_weight"],"metrics":{**fm,"control_mean_prediction_delta_abs":float(imp.mean()),"control_percentile_midrank":pct,"empirical_p":emp,"bh_adjusted_p":None,"effect_difference_bootstrap_ci":np.quantile(dist,[alpha/2,1-alpha/2]).tolist()},"prediction_file":str(pred_path.relative_to(out)),"controls_file":str(control_path.relative_to(out)),"control_seed":seed})
    ...
    catalog={"run_id":run_id,"status":"complete","dataset":"ETTh1","protocol":"global_structural_edge_removal","schedule":{"state":"final","current_epoch_equivalent":5,"static_weight":.1,"learned_weight":.9},"case_count":len(records),"cases":records,"cross_run":{"status":"missing","metrics":None,"reason":"Only one real ETTh1 checkpoint is available."}}
