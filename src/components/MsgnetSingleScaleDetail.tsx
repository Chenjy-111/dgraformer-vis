...
export function MsgnetSingleScaleDetail({catalog}:{catalog:MsgnetCatalog}) {
  ...
  const across=catalog.samples.map((item,index)=>({index,impact:item.edge_impacts.find(value=>value.scale_index===scale&&value.source===selected.source&&value.target===selected.target)}));
  ...
  return <main className="mx-auto max-w-[1240px] space-y-5 px-5 py-10">
    <section className="card grid gap-4 p-5 md:grid-cols-4">
      <Choice label="Test sample" value={String(samplePosition)} change={value=>{setSamplePosition(+value);setEdgeId('')}} options={catalog.samples.map((item,index)=>[String(index),`ETTh1 · test ${index} · source ${item.sample_index}`])}/>
