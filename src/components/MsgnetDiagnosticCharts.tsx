...
export function ImpactBars({items,selected}:{items:MsgnetEdgeImpact[];selected:MsgnetEdgeImpact}){
 ...
 return <div><svg viewBox="0 0 720 190" className="w-full rounded-lg border border-line bg-white" role="img" aria-label="Prediction response across audited tests">{items.map((item,index)=>{const height=item.prediction_delta_abs/max*125;return <g key={index}><rect x={65+index*125} y={150-height} width="50" height={height} rx="3" fill={item===selected?'#d65b52':'#16827f'}/><text x={90+index*125} y="170" textAnchor="middle" fontSize="10" fill="#657286">test {index}</text><text x={90+index*125} y={Math.max(14,145-height)} textAnchor="middle" fontSize="9" fill="#34445a">{item.prediction_delta_abs.toFixed(4)}</text></g>})}</svg><p className="mt-2 text-[10px] text-ink-400">Bar height is mean absolute prediction change. The selected test is highlighted.</p></div>
}
