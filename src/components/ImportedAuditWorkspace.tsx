...
export function ImportedEvidenceWorkspace() {
  ...
  return <div className="mx-auto max-w-[1240px] space-y-5 px-5 py-8">
    ...
    <div className="grid gap-5 lg:grid-cols-2"><EvidenceCard title={`Local ${pending.contextType} evidence`} record={local} sample={sample} target={pending.target}/><EvidenceCard title="Broader-context evidence" record={broader} sample={sample} target={pending.target}/></div>
...
function EvidenceCard({ title, record, sample, target }: { title: string; record?: AuditEvidenceRecord; sample?: AuditSession['samples'][number]; target: number }) {
  if (!record) return <section className="card p-5"><div className="flex items-center gap-2 text-amber-700"><CircleOff size={16}/><h3 className="text-[15px] font-semibold">{title}</h3></div><p className="mt-4 rounded-lg bg-amber-50 p-4 text-[11px] text-amber-800">No evidence available for this exact selection.</p></section>;
