...
function DgraSummary({ session, group, local, all, localBundles, selectedWindow, onSelectWindow }: { session: AuditSessionV2; group: ReturnType<typeof relationGroups>[number]; local: CandidateBundle | null; all: CandidateBundle | null; localBundles: Array<{ context: number; bundle: CandidateBundle | null }>; selectedWindow: number | null; onSelectWindow: (value: number) => void }) {
  ...
  return <section className="space-y-5"><RelationAuditHeader relation={`${group.sourceName} → ${group.targetName}`} model="DGraFormer" localContext={`W${selectedWindow ?? '—'}`} contextCount={`${group.retained.length} retained window${group.retained.length === 1 ? '' : 's'}`} local={local} global={all} equivalent={!multiple}/>{!multiple && <EquivalentScopeNotice/>}<SelectedScopeComparison local={local} global={all ?? (!multiple ? local : null)} localLabel={`Selected Single Window · W${selectedWindow ?? '—'}`} globalLabel="All Retained Windows" equivalent={!multiple}/><MethodSensitivity local={local} global={all} localLabel={multiple ? `Local · W${selectedWindow}` : `Single window · W${selectedWindow}`} globalLabel="All retained windows"/><ProvenancePanel session={session} local={local} global={all} localLabel={`Single window · W${selectedWindow}`} globalLabel="All retained windows"/><ScopeEvidenceMap relation={`${group.sourceName} → ${group.targetName}`} items={mapItems} equivalent={!multiple}/></section>;
}
...
function MsgnetSummary({ session, single, all, scaleBundles, selectedScale, onSelectScale }: { session: AuditSessionV2; single: CandidateBundle | null; all: CandidateBundle | null; scaleBundles: Array<{ context: number; bundle: CandidateBundle | null }>; selectedScale: number; onSelectScale: (value: number) => void }) {
  ...
  return <section className="space-y-5"><RelationAuditHeader relation={`${candidate.source_name} → ${candidate.target_name}`} model="MSGNet" localContext={`scale_index ${selectedScale}`} contextCount={`${scaleBundles.length} scales`} local={single} global={all}/><SelectedScopeComparison local={single} global={all} localLabel={`Single-scale · scale_index ${selectedScale}`} globalLabel="All-scale"/><MethodSensitivity local={single} global={all} localLabel={`Single-scale · scale_index ${selectedScale}`} globalLabel="All-scale"/><ProvenancePanel session={session} local={single} global={all} localLabel={`Single-scale · scale_index ${selectedScale}`} globalLabel="All-scale"/><ScopeEvidenceMap relation={`${candidate.source_name} → ${candidate.target_name}`} items={mapItems}/></section>;
}
