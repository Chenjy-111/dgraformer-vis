...
import { exactCandidate, exactCase, loadBuiltInSessionV2, relationGroups, type CandidateBundle } from '@/data/auditSessionV2View';
...
export function DgraSessionV2Evidence({ supplied }: { supplied?: AuditSessionV2 | null }) {
  ...
  useEffect(() => {
    const bundle = multiple ? local : all ?? local;
    if (!bundle) { setSummarySampleId(null); return; }
    setSummarySampleId(current => current !== null && bundle.evidence.planned_samples.includes(current) ? current : bundle.evidence.planned_samples[0] ?? null);
  }, [local?.candidate.candidate_id, all?.candidate.candidate_id, multiple]);
  ...
  return <section id="dgra-session-v2-evidence" className="mx-auto max-w-[1400px] space-y-5 px-5 pb-14">
    ...
    {group && <>{multiple && <section className="card flex flex-wrap items-center gap-3 p-4"><span className="text-[11px] font-semibold uppercase tracking-wider text-ink-400">Local window</span>{group.retained.map(value => <button key={value} onClick={() => setLocalWindow(value)} className={`rounded-md px-3 py-2 text-[11px] font-semibold ${value === windowId ? 'bg-[#16827f] text-white' : 'bg-[#edf1f5] text-ink-600 hover:bg-[#dfe7ed]'}`}>W{value}</button>)}<span className="text-[11px] text-ink-400">Frozen retained-window order; independent of outcomes.</span></section>}
      ...
      {tab === 'summary' && <DgraSummary session={session} group={group} local={local} all={all} localBundles={localBundles} selectedWindow={windowId} onSelectWindow={setLocalWindow} sampleId={summarySampleId} setSampleId={setSummarySampleId}/>}
...
export function MsgnetSessionV2Evidence({ supplied }: { supplied?: AuditSessionV2 | null }) {
  ...
  useEffect(() => {
    const bundle = single ?? all;
    if (!bundle) { setSummaryTest(null); return; }
    setSummaryTest(current => current !== null && bundle.evidence.planned_samples.includes(current) ? current : bundle.evidence.planned_samples[0] ?? null);
  }, [single?.candidate.candidate_id, all?.candidate.candidate_id]);
  ...
  return <section id="msgnet-session-v2-evidence" className="mx-auto max-w-[1400px] space-y-5 px-5 pb-14">
    ...
    {tab === 'summary' && <MsgnetSummary session={session} single={single} all={all} scaleBundles={scaleBundles} selectedScale={selected.contextIndex} onSelectScale={setLocalScale} testId={summaryTest} setTestId={setSummaryTest}/>}
...
function DgraSummary({ session, group, local, all, localBundles, selectedWindow, onSelectWindow, sampleId, setSampleId }: { session: AuditSessionV2; group: ReturnType<typeof relationGroups>[number]; local: CandidateBundle | null; all: CandidateBundle | null; localBundles: Array<{ context: number; bundle: CandidateBundle | null }>; selectedWindow: number | null; onSelectWindow: (value: number) => void; sampleId: number | null; setSampleId: (value: number) => void }) {
  const multiple = group.retained.length > 1, chartBundle = multiple ? local : all ?? local;
  ...
  return <section className="space-y-5"><RelationAuditHeader relation={`${group.sourceName} → ${group.targetName}`} model="DGraFormer" localContext={`W${selectedWindow ?? '—'}`} contextCount={`${group.retained.length} retained window${group.retained.length === 1 ? '' : 's'}`} audited={Boolean(local || all)}/>{!multiple && <EquivalentScopeNotice/>}<SelectedScopeComparison local={local} global={all ?? (!multiple ? local : null)} localLabel={`Selected Single Window · W${selectedWindow ?? '—'}`} globalLabel="All Retained Windows" equivalent={!multiple}/><IllustrativeCase session={session} target={group.target} ids={chartBundle?.evidence.planned_samples ?? []} sampleId={sampleId} setSampleId={setSampleId} records={[[multiple ? `Single-window Removal · W${selectedWindow}` : 'Relation Removed', chartCase]]} protocolText="Formal inference uses the complete frozen sample protocol."/><MethodSensitivity local={local} global={all} localLabel={multiple ? `Local · W${selectedWindow}` : 'Equivalent intervention'} globalLabel="All retained windows"/><ProvenancePanel session={session} bundle={local ?? all}/><ScopeEvidenceMap relation={`${group.sourceName} → ${group.targetName}`} items={mapItems} equivalent={!multiple}/></section>;
}

function MsgnetSummary({ session, single, all, scaleBundles, selectedScale, onSelectScale, testId, setTestId }: { session: AuditSessionV2; single: CandidateBundle | null; all: CandidateBundle | null; scaleBundles: Array<{ context: number; bundle: CandidateBundle | null }>; selectedScale: number; onSelectScale: (value: number) => void; testId: number | null; setTestId: (value: number) => void }) {
  ...
  const singleCase = single && testId !== null ? exactCase(session, single.candidate, testId) : null, allCase = all && testId !== null ? exactCase(session, all.candidate, testId) : null;
